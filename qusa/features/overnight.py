...
class OvernightCalculator: 
    ...
    def calculate_overnight_delta(self, df): 
        ...
        df_mod[self.date] = pd.to_datetime(df_mod[self.date])  # confirm date column as datetime type
        df_mod = df_mod.sort_values(by=self.date).reset_index(drop=True)  # sort by date and reset index
